...
header="Patient_ID"
for i in ${!labels[@]}; do
    header="$header,${labels[$i]}_volume,${labels[$i]}_intensity"
done
echo $header > $stats_output

# Export necessary variables and functions for xargs
export input_dir output_base_dir stats_output compressed_dir labels
export -f process_patient.sh

# Process each patient in parallel using xargs
ls $input_dir | xargs -n 1 -P 40 ./process_patient.sh
